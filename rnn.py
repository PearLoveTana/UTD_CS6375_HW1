...
class RNN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        output_sums = torch.sum(outputs, dim = 0)
        # [to fill] sum over output 
        predicted_vector = self.softmax(self.W(output_sums))
...
if __name__ == "__main__":
    ...
    for minibatch_index in tqdm(range(N // minibatch_size)):
        for example_index in range(minibatch_size):
            input_words, gold_label = test_data[minibatch_size * minibatch_index + example_index]
            input_words = " ".join(input_words)
            input_words = input_words.translate(input_words.maketrans("","",string.punctuation)).split()
            vectors = [word_embedding[i.lower()] if i.lower() in word_embedding.keys() else word_embedding['unk'] for i in input_words]
            vectors = torch.tensor(vectors).view(len(vectors), 1, -1)
    ...
    ax1.plot(epoch_list, train_loss_list, color=color, marker='o', label="Train Loss")
    ax1.tick_params(axis='y', labelcolor=color)

    ax2 = ax1.twinx()  # Instantiate a second axes that shares the same x-axis
    color = 'tab:blue'
    ax2.set_ylabel('Validation Accuracy', color=color)
    ax2.plot(epoch_list, val_acc_list, color=color, marker='o', label="Val Accuracy")
